...
export const RatingSection: React.FC = () => {
  ...
  const metrics = [
    { value: "4.9", unit: "/5", label: t.rating.ux, detail: t.rating.uxDetail },
    { value: "100", unit: "%", label: t.rating.reliability, detail: t.rating.relDetail },
    { value: "<1", unit: "ms", label: t.rating.latency, detail: t.rating.latDetail }
  ];
    ...
    <section className="py-32 bg-alabaster dark:bg-obsidian border-t border-obsidian/5 dark:border-white/5 transition-colors duration-500">
      <div className="max-w-7xl mx-auto px-6">
        <div className="text-center mb-20">
          ...
          <h2 className="text-4xl md:text-6xl font-black tracking-tighter text-obsidian dark:text-white uppercase leading-none">{t.rating.title}</h2>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-8">
          {[
            {
              quote: language === 'en' 
                ? "The transition to Konnect Fiber lb was seamless. Gigabit speed in Chekka has completely changed how I work."
                : "كان الانتقال إلى Konnect Fiber lb سلساً للغاية. لقد غيرت سرعة الجيجابت في شكا طريقة عملي تماماً.",
              author: "Elite Node #428",
              location: language === 'en' ? "Chekka Residential" : "شكا السكنية"
            },
            {
              quote: language === 'en'
                ? "Unlimited Nights is the differentiator. I can sync massive datasets at 3 AM with zero latency issues."
                : "الليالي غير المحدودة هي الميزة الفارقة. يمكنني مزامنة مجموعات بيانات ضخمة في الساعة 3 صباحاً دون مشاكل استجابة.",
              author: "Business Client #012",
              location: language === 'en' ? "North Enterprise Hub" : "مركز أعمال الشمال"
            }
          ].map((testimonial, idx) => (
            <div 
              key={idx} 
